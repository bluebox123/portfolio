import React, { useEffect, useRef } from 'react';

/* ───────────────────────────────────────────────────────────────────────────
   SEASON ATMOSPHERE — grades the WHOLE page through the year as you scroll.

     BACKDROP (z-0) — dark colour-graded gradient behind the (translucent)
       content; mood interpolates between the two live seasons.
     CANVAS  (z-30) — the weather:
       identity=RAIN capabilities=AUTUMN experience=WINTER projects=SPRING contact=SUMMER

   DENSITY ENVELOPE:
     • Scrolling INTO a section → a DENSE flood of that season pours in from
       above (summer rises from below), then GRADUALLY THINS to a light ambient
       level as you settle into the section.
     • Scrolling OUT (incl. scroll-up) → the leaving season's particles SWEEP
       apart to the left and right and clear off the edges.
   Everything is eased and direction-aware, so it reads like wind — no snaps.

   PERF: section centres cached (re-measured only on layout-height change);
   density thins by drawing fewer particles; capped DPR; one rAF.
   ─────────────────────────────────────────────────────────────────────────── */

const prefersReduced = () =>
  typeof window !== 'undefined' &&
  window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;

// scroll order = season order
const SECTION_IDS = ['identity', 'capabilities', 'experience', 'projects', 'contact'];
type Kind = 'rain' | 'autumn' | 'winter' | 'spring' | 'summer';
const KINDS: Kind[] = ['rain', 'autumn', 'winter', 'spring', 'summer'];

// pool size = the FULL flood density; ambient draws a fraction of it
const COUNT: Record<Kind, number> = { rain: 300, autumn: 195, winter: 375, spring: 205, summer: 135 };

const SWEEP = 820;        // px/s sideways blow-away at full exit
const BASE_DENS = 0.3;    // ambient density once the flood has settled
const INTENSITY_DECAY = 0.28; // per second — how fast the flood thins out

// dark backdrop gradient stops per season (top, bottom) — low luminance, keeps text crisp
const BG_TOP: [number, number, number][] = [
  [12, 16, 22], [24, 15, 8], [10, 16, 24], [20, 11, 18], [24, 18, 9],
];
const BG_BOT: [number, number, number][] = [
  [4, 6, 9], [9, 5, 3], [4, 7, 11], [8, 4, 8], [9, 7, 4],
];

const AUTUMN = ['#C2410C', '#B45309', '#9A3412', '#C9A962', '#D97706'];
const SPRING = ['#F9A8C4', '#F4A6C0', '#FBCFE8', '#F472B6', '#FCA5A5'];

interface P {
  x: number; y: number; vx: number; vy: number;
  s: number; rot: number; vrot: number; ph: number; sw: number;
  a: number; len: number; col: string;
}

const rnd = (a: number, b: number) => a + Math.random() * (b - a);
const pick = (arr: string[]) => arr[(Math.random() * arr.length) | 0];

function spawn(kind: Kind, w: number, h: number, seed = false): P {
  switch (kind) {
    case 'rain':
      return {
        x: rnd(-0.1 * w, 1.1 * w), y: seed ? rnd(0, h) : -rnd(0, h),
        vx: rnd(-90, -40), vy: rnd(820, 1250), s: 0, rot: 0, vrot: 0,
        ph: 0, sw: 0, a: rnd(0.12, 0.34), len: rnd(12, 26), col: '#A9CCE3',
      };
    case 'autumn':
      return {
        x: rnd(0, w), y: seed ? rnd(0, h) : rnd(-40, -10),
        vx: rnd(-12, 8), vy: rnd(45, 95), s: rnd(5, 11),
        rot: rnd(0, Math.PI * 2), vrot: rnd(-1.3, 1.3), ph: rnd(0, 6.28),
        sw: rnd(22, 52), a: rnd(0.55, 0.92), len: 0, col: pick(AUTUMN),
      };
    case 'winter':
      return {
        x: rnd(0, w), y: seed ? rnd(0, h) : rnd(-40, -10),
        vx: 0, vy: rnd(28, 62), s: rnd(1.3, 3.8),
        rot: 0, vrot: 0, ph: rnd(0, 6.28), sw: rnd(8, 20),
        a: rnd(0.45, 0.85), len: 0, col: '#EAF2FB',
      };
    case 'spring':
      return {
        x: rnd(0, w), y: seed ? rnd(0, h) : rnd(-40, -10),
        vx: rnd(-10, 10), vy: rnd(38, 78), s: rnd(4, 9),
        rot: rnd(0, Math.PI * 2), vrot: rnd(-1.6, 1.6), ph: rnd(0, 6.28),
        sw: rnd(26, 58), a: rnd(0.5, 0.85), len: 0, col: pick(SPRING),
      };
    default: // summer
      return {
        x: rnd(0, w), y: seed ? rnd(0, h) : rnd(h, h + 40),
        vx: rnd(-14, 14), vy: rnd(-46, -16), s: rnd(1.3, 2.6),
        rot: 0, vrot: 0, ph: rnd(0, 6.28), sw: rnd(10, 22),
        a: rnd(0.45, 0.9), len: 0, col: Math.random() < 0.5 ? '#FFD27A' : '#C9A962',
      };
  }
}

export default function SeasonAtmosphere() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const backdropRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    if (!ctx) return;
    const reduce = prefersReduced();

    let W = window.innerWidth || 1;
    let H = window.innerHeight || 1;

    const pools: Record<Kind, P[]> = { rain: [], autumn: [], winter: [], spring: [], summer: [] };
    const build = () => {
      for (const k of KINDS) pools[k] = Array.from({ length: COUNT[k] }, () => spawn(k, W, H, true));
    };

    // re-seed a particle at its season's entrance band (top for most, below for summer)
    const reseed = (p: P, kind: Kind) => {
      p.x = rnd(0, W);
      p.y = kind === 'summer' ? rnd(H + 10, H * 1.6) : rnd(-H * 0.7, -10);
      p.ph = rnd(0, 6.28);
    };
    const enter = (kind: Kind) => { for (const p of pools[kind]) reseed(p, kind); };

    // cached section centres (document coords) — re-measured only on layout change
    let centers: number[] = [];
    let lastScrollH = -1;
    const measure = () => {
      centers = SECTION_IDS.map((id) => {
        const el = document.getElementById(id);
        if (!el) return NaN;
        const r = el.getBoundingClientRect();
        return r.top + window.scrollY + r.height / 2;
      });
      lastScrollH = document.documentElement.scrollHeight;
    };

    const resize = () => {
      W = window.innerWidth || 1; H = window.innerHeight || 1;
      const dpr = Math.min(1.5, window.devicePixelRatio || 1);
      canvas.width = Math.floor(W * dpr);
      canvas.height = Math.floor(H * dpr);
      canvas.style.width = W + 'px';
      canvas.style.height = H + 'px';
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
      measure();
    };
    resize();
    build();
    window.addEventListener('resize', resize);

    const seasonPos = () => {
      const sh = document.documentElement.scrollHeight;
      if (sh !== lastScrollH) measure();
      const vc = window.scrollY + H / 2;
      const c = centers;
      if (c.length < 5 || c.some(Number.isNaN)) {
        const docH = sh - H;
        return docH > 0 ? Math.min(1, Math.max(0, window.scrollY / docH)) * 4 : 0;
      }
      if (vc <= c[0]) return 0;
      if (vc >= c[4]) return 4;
      for (let i = 0; i < 4; i++) {
        if (vc >= c[i] && vc <= c[i + 1]) return i + (vc - c[i]) / Math.max(1, c[i + 1] - c[i]);
      }
      return 0;
    };

    let last = performance.now();
    let raf = 0;
    let lastSp = seasonPos();
    let dirSign = 1;
    let lastBgSp = -999;
    const prevW = [0, 0, 0, 0, 0];
    const intensity = [0, 0, 0, 0, 0]; // flood level per season, decays toward 0
    let primed = false;

    const draw = (now: number) => {
      const dt = Math.min(0.05, (now - last) / 1000);
      last = now;
      const el = now / 1000;
      const sp = seasonPos();

      const dv = sp - lastSp;
      lastSp = sp;
      if (dv > 0.0008) dirSign = 1; else if (dv < -0.0008) dirSign = -1;

      // backdrop mood — only rewrite when the season meaningfully shifts
      if (backdropRef.current && Math.abs(sp - lastBgSp) > 0.01) {
        const i0 = Math.max(0, Math.min(4, Math.floor(sp)));
        const i1 = Math.min(4, i0 + 1);
        const fr = sp - i0;
        const mix = (a: number, b: number) => Math.round(a + (b - a) * fr);
        const tt = `rgb(${mix(BG_TOP[i0][0], BG_TOP[i1][0])},${mix(BG_TOP[i0][1], BG_TOP[i1][1])},${mix(BG_TOP[i0][2], BG_TOP[i1][2])})`;
        const bb = `rgb(${mix(BG_BOT[i0][0], BG_BOT[i1][0])},${mix(BG_BOT[i0][1], BG_BOT[i1][1])},${mix(BG_BOT[i0][2], BG_BOT[i1][2])})`;
        backdropRef.current.style.background = `linear-gradient(180deg, ${tt} 0%, ${bb} 100%)`;
        lastBgSp = sp;
      }

      ctx.clearRect(0, 0, W, H);

      for (let ki = 0; ki < KINDS.length; ki++) {
        const kind = KINDS[ki];
        const d = sp - ki;
        const ad = Math.abs(d);
        const weight = Math.max(0, 1 - ad);
        const exiting = (dirSign >= 0 && d > 0) || (dirSign < 0 && d < 0);
        const sweepRaw = exiting ? Math.min(1, ad) : 0;
        const sweep = sweepRaw * sweepRaw; // ease-in

        // entrance: a fresh, visible season floods in dense from above
        if (primed && weight >= 0.04 && prevW[ki] < 0.04 && !exiting) {
          if (kind !== 'rain') enter(kind);
          intensity[ki] = 1;
        }
        prevW[ki] = weight;

        // flood thins gradually toward the ambient baseline
        if (intensity[ki] > 0) intensity[ki] = Math.max(0, intensity[ki] - dt * INTENSITY_DECAY);

        // visible strength: crossfade weight LIFTED by the entry flood, so the
        // dense burst is bright immediately even while the section's weight is
        // still rising (otherwise the flood pours in at ~0 opacity, invisibly)
        const vis = Math.min(1, weight + intensity[ki]);
        if (vis <= 0.01) continue;
        const arr = pools[kind];

        // how many particles to draw right now (dense on entry, sparse at rest)
        const dens = BASE_DENS + (1 - BASE_DENS) * intensity[ki];
        const af = arr.length * dens; // fractional active count for a soft thinning edge

        if (kind === 'rain') {
          ctx.lineCap = 'round';
          ctx.lineWidth = 1.1;
          ctx.strokeStyle = '#A9CCE3';
          for (let pi = 0; pi < arr.length; pi++) {
            const vizf = af - pi; if (vizf <= 0) break;
            const p = arr[pi];
            p.x += p.vx * dt; p.y += p.vy * dt;
            if (p.y > H + 20) { p.y = -20; p.x = rnd(-0.1 * W, 1.1 * W); }
            if (p.x < -40) p.x = W + 40;
            ctx.globalAlpha = p.a * vis * Math.min(1, vizf);
            ctx.beginPath();
            ctx.moveTo(p.x, p.y);
            ctx.lineTo(p.x - p.vx * 0.018, p.y - p.len);
            ctx.stroke();
          }
          ctx.globalAlpha = 1;
        } else if (kind === 'winter') {
          ctx.fillStyle = '#EAF2FB';
          for (let pi = 0; pi < arr.length; pi++) {
            const vizf = af - pi; if (vizf <= 0) break;
            const p = arr[pi];
            const side = p.x < W * 0.5 ? -1 : 1;
            p.x += (Math.sin(el * 1.1 + p.ph) * p.sw + side * SWEEP * sweep) * dt;
            p.y += p.vy * (1 - sweep * 0.5) * dt;
            if (p.x < -50 || p.x > W + 50) reseed(p, kind);
            else if (p.y > H + 10) reseed(p, kind);
            ctx.globalAlpha = p.a * vis * Math.min(1, vizf);
            ctx.beginPath();
            ctx.arc(p.x, p.y, p.s, 0, 6.2832);
            ctx.fill();
          }
          ctx.globalAlpha = 1;
        } else if (kind === 'summer') {
          for (let pi = 0; pi < arr.length; pi++) {
            const vizf = af - pi; if (vizf <= 0) break;
            const p = arr[pi];
            const side = p.x < W * 0.5 ? -1 : 1;
            p.x += (Math.sin(el * 0.8 + p.ph) * p.sw + p.vx + side * SWEEP * sweep) * dt;
            p.y += p.vy * dt;
            if (p.x < -50 || p.x > W + 50) reseed(p, kind);
            else if (p.y < -20) reseed(p, kind);
            const pulse = 0.4 + 0.6 * (0.5 + 0.5 * Math.sin(el * 2.2 + p.ph));
            const aFull = Math.min(1, p.a * vis * pulse * Math.min(1, vizf));
            ctx.fillStyle = p.col;
            ctx.globalAlpha = aFull * 0.32;          // soft halo
            ctx.beginPath();
            ctx.arc(p.x, p.y, p.s * 3.2, 0, 6.2832);
            ctx.fill();
            ctx.globalAlpha = aFull;                  // bright core
            ctx.beginPath();
            ctx.arc(p.x, p.y, p.s * 1.1, 0, 6.2832);
            ctx.fill();
            ctx.globalAlpha = 1;
          }
        } else {
          // autumn leaves + spring petals — rotated ellipse (no save/restore)
          const ry = kind === 'spring' ? 0.42 : 0.58;
          for (let pi = 0; pi < arr.length; pi++) {
            const vizf = af - pi; if (vizf <= 0) break;
            const p = arr[pi];
            const side = p.x < W * 0.5 ? -1 : 1;
            p.x += (Math.sin(el * 1.4 + p.ph) * p.sw + p.vx + side * SWEEP * sweep) * dt;
            p.y += p.vy * (1 - sweep * 0.5) * dt;
            p.rot += (p.vrot + side * sweep * 3) * dt;
            if (p.x < -50 || p.x > W + 50) reseed(p, kind);
            else if (p.y > H + 20) reseed(p, kind);
            ctx.globalAlpha = Math.min(1, p.a * vis * Math.min(1, vizf));
            ctx.fillStyle = p.col;
            ctx.beginPath();
            ctx.ellipse(p.x, p.y, p.s, p.s * ry, p.rot, 0, 6.2832);
            ctx.fill();
          }
          ctx.globalAlpha = 1;
        }
      }
      primed = true;

      if (!reduce) raf = requestAnimationFrame(draw);
    };

    if (reduce) {
      draw(performance.now());
      return () => window.removeEventListener('resize', resize);
    }

    raf = requestAnimationFrame(draw);

    // self-heal if mounted at 0×0 (headless preview) — rebuild once a real size appears
    const healId = window.setInterval(() => {
      if ((window.innerWidth && window.innerWidth !== W) ||
          (window.innerHeight && window.innerHeight !== H)) {
        resize(); build();
      }
    }, 600);

    return () => {
      cancelAnimationFrame(raf);
      window.clearInterval(healId);
      window.removeEventListener('resize', resize);
    };
  }, []);

  return (
    <>
      <div ref={backdropRef} className="season-backdrop" aria-hidden="true" />
      <canvas ref={canvasRef} className="season-canvas" aria-hidden="true" />
    </>
  );
}
